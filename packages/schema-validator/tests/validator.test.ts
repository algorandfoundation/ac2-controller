...
import { AC2MessageTypes } from "../src/types.js";
import { validate, validateBody } from "../src/validator.js";
...
describe("validate() — base envelope", () => {
  ...
  it("warns when @context is absent", () => {
    const { "@context": _, ...noCtx } = validSigningRequest;
    const r = validate(noCtx);
    expect(r.valid).toBe(true);
    expect(r.warnings.some((w) => w.includes("@context"))).toBe(true);
  });

  it("warns when @context does not include ac2 namespace", () => {
    const r = validate({ ...validSigningRequest, "@context": ["https://example.com"] });
    expect(r.valid).toBe(true);
    expect(r.warnings.some((w) => w.includes("@context"))).toBe(true);
  });
